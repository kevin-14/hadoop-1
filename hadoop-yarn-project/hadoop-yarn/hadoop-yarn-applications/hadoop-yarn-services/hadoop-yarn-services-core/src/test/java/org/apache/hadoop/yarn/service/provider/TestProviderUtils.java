...
public class TestProviderUtils {
  ...
  public void testStaticFileLocalization() throws IOException {
    ...
    when(comp.getConfiguration()).thenReturn(conf);
    ...
    ProviderUtils.handleStaticFilesForLocalization(launcher, sfs, comp);
    Mockito.verify(launcher).addLocalResource(Mockito.eq("localized/destFile1"),
        any(LocalResource.class));
    Mockito.verify(launcher).addLocalResource(
        Mockito.eq("localized/destFile_2"), any(LocalResource.class));
    Mockito.verify(launcher).addLocalResource(
        Mockito.eq("localized/sourceFile4"), any(LocalResource.class));
  }
}

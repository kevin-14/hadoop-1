...
=======
|type|Config file in the standard format like xml, properties, json, yaml, template. When STATIC/ARCHIVE is specified, file must be uploaded to remote file system before launch the job, and YARN service framework will localize files prior to launch containers|false|enum (XML, PROPERTIES, JSON, YAML, TEMPLATE, ENV, HADOOP_XML,STATIC,ARCHIVE)||
>>>>>>> theirs
